...
class Portfolio():
    """
    A portfolio object containing information about holdings, market value and returns.
    ...
    def __init__(self, holdings: pd.DataFrame, cashflows: pd.Series):
        ...
        full_range = pd.date_range(start=holdings.index.min(),
                           end=(pd.Timestamp.today() - dt.timedelta(1)).normalize(), freq='D')
        
        # Initiate attributes
        self.holdings = holdings.reindex(full_range).ffill()
        _holds = self.holdings.iloc[-1]
        self._holds = _holds[_holds != 0]  # Internal use, non-0 current holdings
    ...
    def from_etoro(cls, acc_activity: pd.DataFrame):
        """
        Initialize a Portfolio instance by loading an etoro 'Account Activity' dataframe.
        ...
        # Track cashflows 
        cashflows = acc_activity.loc[acc_activity['Type']=='Deposit', 'Amount'].copy()
        ...
        trades = acc_activity.loc[acc_activity['Type'].isin(transactions)].copy()
        ...
        trades = trades.groupby([trades.index, 'Asset'])['Change'].sum().reset_index()

        # Pivot to have assets as columns
        trades = trades.pivot(index='Date', columns='Asset', values='Change')
        trades = trades.resample('D').sum()  # Resample to total end of day changes

        # Adjust for stock splits
        trades = Portfolio._adjust_splits(trades=trades, acc_activity=acc_activity)
        ...
        holdings = holdings.rename({'Balance':'USD'}, axis=1)  # Rename balance col
    ...
    def _convert_etoro_tickers(series: pd.Series):
        '''Convert mapped eToro tickers to LSEG, or returns a list of tickers to add to map'''
    ...
    def _adjust_splits(trades: pd.DataFrame, acc_activity: pd.DataFrame):
        """Augments the trades DataFrame for stock splits based on account activity.

        Parameters
        ----------
        trades : pd.DataFrame
            DataFrame of trade unit changes, indexed by Date and with assets as columns.
        ...
        # Locate stock splits
        splits = acc_activity.loc[acc_activity['Type']=='corp action: Split']\
        ...
        splits['Numerator'] = splits['Details'].apply(lambda x: int(x.split(' ')[1].split(':')[0]))
        splits['Denominator'] = splits['Details'].apply(lambda x: int(x.split(' ')[1].split(':')[1]))
        splits['Multiplier'] = splits['Numerator'] / splits['Denominator']

        # Drop unnecessary cols
        splits = splits[['Date', 'Asset', 'Multiplier']]
        splits['Date'] = splits['Date'].dt.normalize()  # Set as only days (no hours)

        # Handle edge case where there are multiple splits in a day
        splits = splits.groupby(['Date', 'Asset'])['Multiplier'].prod().reset_index()

        # Add trades for splits
        for _, split_r in splits.iterrows():
            
            # Get current holdings of the asset (t-1)
            held = trades.loc[:split_r['Date'], split_r['Asset']].cumsum().iloc[-1]
